...
/* Firestore rejects any field whose value is literally `undefined`
   (it throws, and the whole save fails) — but it's easy for a form
   field to end up undefined (an unset <select>, a destructured value
   with no match, etc.). Rather than track down every call site, every
   write path below runs its data through this first: undefined becomes
   null (which Firestore accepts fine), recursively through arrays and
   nested objects like product variants. */
...
/* Rewrite a Cloudinary delivery URL to request an auto-optimized,
   appropriately-sized asset (format + quality auto, optional width cap)
   instead of the full original upload. No-ops for non-Cloudinary URLs
   or URLs that already carry a transformation. */
function cldOptimize(url, transform){
  if(!url || typeof url !== 'string') return url;
  if(!transform) transform = 'f_auto,q_auto';
  if(!url.includes('res.cloudinary.com') || !url.includes('/upload/')) return url;
  if(/\/upload\/[^/]*[wqf]_/.test(url)) return url; // already transformed
  return url.replace('/upload/', `/upload/${transform}/`);
}

function mediaTagHTML(url, alt, extraAttrs, cldTransform, eager){
  extraAttrs = extraAttrs || '';
  if(isVideoUrl(url)){
    return `<span class="media-video-wrap">
      <video src="${url}" muted loop playsinline autoplay preload="metadata" ${extraAttrs}></video>
      <button type="button" class="media-unmute-btn" aria-label="Toggle sound">🔇</button>
    </span>`;
  }
  const optimized = cldOptimize(url, cldTransform);
  const loadingAttr = eager ? '' : 'loading="lazy"';
  return `<img src="${optimized}" alt="${alt||''}" ${loadingAttr} decoding="async" ${extraAttrs}>`;
}

/* Any video rendered via mediaTagHTML() starts muted (required for
   autoplay). This single delegated listener lets the person tap the
   speaker icon on ANY such video — product gallery, cards, etc. —
   to turn its sound on, and mutes it back on a second tap.
   Capture phase + stopPropagation so this fires BEFORE a wrapping
   card's own onclick (e.g. productCardHTML's goToProduct navigation),
   otherwise tapping the unmute icon would also navigate away. */
document.addEventListener('click', e=>{
  const btn = e.target.closest('.media-unmute-btn');
  if(!btn) return;
  e.stopPropagation();
  const wrap = btn.closest('.media-video-wrap');
...
async function uploadFileToCloudinary(file, onProgress){
  ...
  // Large video/audio files on slow mobile data need real headroom — 180s
  // was cutting off legitimate in-progress uploads. Images stay short since
  // they're small and a stuck image upload usually means a dead connection.
  const timeoutMs = isImage ? 120000 : 600000; // 2 min images, 10 min video/audio
...
(async function initFirebase(){
  ...
  VDB = {
    ...
    async saveProduct(data){
      const toSave = sanitizeForFirestore({...data});
  ...
  try{
    ...
    async uploadHeroMedia(file, onProgress){
      ...
      try{
        ...
        throw new Error(`Failed to upload hero media: ${err.message}`);
      }
    },
    ...
    async uploadFounderMedia(file, onProgress){
      const type = file.type.startsWith('video/') ? 'video' : 'image';
      try{
        const url = await uploadFileToCloudinary(file, onProgress);
        const founderMedia = { url, type };
        await setDoc(doc(db,"settings","store"),{ founderMedia },{ merge:true });
        return founderMedia;
      } catch(err){
        throw new Error(`Failed to upload founder media: ${err.message}`);
      }
    },
    async removeFounderMedia(){
      await setDoc(doc(db,"settings","store"),{ founderMedia: null },{ merge:true });
    },
    subscribeFounderMedia(cb){
      return onSnapshot(doc(db,"settings","store"), s=>{
        cb(s.exists() ? (s.data().founderMedia || null) : null);
      });
    },
    ...
    async uploadPromoImage(file, onProgress){
      try{
        ...
        throw new Error(`Failed to upload promo image: ${err.message}`);
      }
    },
    async savePromoBanner(data){
      await setDoc(doc(db,"settings","store"),{ promoBanner: sanitizeForFirestore(data) },{ merge:true });
    },
    ...
    async saveOrder(data){
      const toSave = sanitizeForFirestore({ ...data, createdAt: new Date().toISOString() });
    ...
    async getOrderByRef(ref2){
      const snap = await getDocs(query(collection(db,"orders")));
      let found = null;
      snap.forEach(d=>{ if(d.data().ref===ref2) found = {id:d.id,...d.data()}; });
      return found;
    },
    ...
    async saveCategory(data){
      const toSave = sanitizeForFirestore({ ...data });
      if(data.id){ const id=String(data.id); delete toSave.id; await setDoc(doc(db,"categories",id),toSave,{merge:true}); return id; }
      delete toSave.id;
    ...
    async addReview(data){
      ...
      const ref2 = await addDoc(collection(db,"reviews"), toSave);
      return ref2.id;
    },
    subscribeReviewsForProduct(productId, cb){
      return onSnapshot(collection(db,"reviews"), snap=>{
        const list=[];
        snap.forEach(d=>{
          const data = d.data();
          if(data.productId===productId && data.approved===true) list.push({id:d.id,...data});
        });
        list.sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt));
        cb(list);
      });
    },
    subscribeAllReviews(cb){
      return onSnapshot(collection(db,"reviews"), snap=>{
        const list=[]; snap.forEach(d=>list.push({id:d.id,...d.data()}));
        list.sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt));
        cb(list);
    ...
    async updateReviewStatus(id, fields){
      await setDoc(doc(db,"reviews",id), fields, { merge:true });
    },
    async deleteReviewById(id){ await deleteDoc(doc(db,"reviews",String(id))); },
    async uploadReviewImage(file, onProgress){
      try{ return await uploadFileToCloudinary(file, onProgress); }
      catch(err){ throw new Error(`Failed to upload review photo: ${err.message}`); }
    },
...
function productCardHTML(p){
  ...
  const img = firstImg ? mediaTagHTML(firstImg, p.name, '', 'f_auto,q_auto,w_500') : `<span>${p.emoji||'🎧'}</span>`;
