...
async function uploadFileToCloudinary(file, onProgress){
  ...
  return new Promise((resolve, reject)=>{
    ...
    xhr.timeout = 180000; // 3 min — mobile MP3 uploads on weak signal need real headroom
...
(async function initFirebase(){
  ...
  const STORAGE_UPLOAD_TIMEOUT_MS = 360000; // 6 minutes — video files need real headroom on mobile data

  /* uploadBytes() gives no progress events at all, which is why the Hero/product
     upload just sat on a static "Uploading..." with no feedback. uploadBytesResumable()
     reports real bytesTransferred/totalBytes as it goes, and .cancel() actually stops
     the transfer when the timeout fires instead of leaving it running in the background. */
